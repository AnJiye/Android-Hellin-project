package com.example.hellinproject.tflite;
...
//    private static final String MODEL_NAME = "mobilenet_imagenet_model.tflite";
    private static final String MODEL_NAME = "squat_model.tflite";
    private static final String LABEL_FILE = "squat_labels.txt";
    ...
    // init() 함수를 빠져나가기 직전에 isInitialized 변수를 true로 설정하여 초기화 여부를 설정
...
//        labels.remove(0);
    ...
    // getModelInputSize() 함수를 호출하는 시점에 아직 모델이 초기화되지 않았을 수도 있기 때문에 boolean 형의 isInitialized 변수를 멤버 변수로 선언하여 모델이 초기화되었는지 확인
    ...
    // 모델의 입력 이미지 크기는 멤버 변수 modelInputWidth, modelInputHeight에 저장되어 있음
